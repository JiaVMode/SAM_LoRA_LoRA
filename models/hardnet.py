    ...
    def __init__(self, depth_wise=False, arch=85, pretrained=True, weight_path='', out=1, args=None):
        ...
        if pretrained:
            ...
            if arch == 68 and not depth_wise:
                local_checkpoint = os.path.join(os.path.dirname(__file__), '..', 'checkpoints', 'hardnet68.pth')
                remote_checkpoint = 'https://ping-chao.com/hardnet/hardnet68-5d684880.pth'
            elif arch == 85 and not depth_wise:
                local_checkpoint = os.path.join(os.path.dirname(__file__), '..', 'checkpoints', 'hardnet85.pth')
                remote_checkpoint = 'https://ping-chao.com/hardnet/hardnet85-a28faa00.pth'
            elif arch == 68 and depth_wise:
                local_checkpoint = os.path.join(os.path.dirname(__file__), '..', 'checkpoints', 'hardnet68ds.pth')
                remote_checkpoint = 'https://ping-chao.com/hardnet/hardnet68ds-632474d2.pth'
            else:
                local_checkpoint = os.path.join(os.path.dirname(__file__), '..', 'checkpoints', 'hardnet39ds.pth')
                remote_checkpoint = 'https://ping-chao.com/hardnet/hardnet39ds-0e6c6fa9.pth'
            
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            
            # 尝试本地加载
            if local_checkpoint and os.path.exists(local_checkpoint):
                print(f"Loading HarDNet from local: {local_checkpoint}")
                self.load_state_dict(torch.load(local_checkpoint, map_location=device))
            elif hasattr(torch, 'hub'):
                # 从网络下载
                print(f"Downloading HarDNet weights (this may take a while)...")
                self.load_state_dict(torch.hub.load_state_dict_from_url(remote_checkpoint, progress=True, map_location=device))
            else:
                postfix = 'ds' if depth_wise else ''
                weight_file = '%shardnet%d%s.pth' % (weight_path, arch, postfix)
                if not os.path.isfile(weight_file):
                    print(f"Warning: {weight_file} not found, using random initialization")
                else:
